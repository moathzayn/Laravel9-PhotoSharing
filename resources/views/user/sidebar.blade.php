<div class="col-md-2 no-padding-left hidden-sm hidden-xs">
    <div class="left-sidebar">
        <div id="sidebar-stick">
            <ul class="menu-sidebar">
                <li><a href="/"><i class="fa fa-home"></i>Home</a></li>
                <li><a href="{{route('upload')}}"><i class="fa fa-upload"></i>upload</a></li>
            </ul>
            <ul class="menu-sidebar">
                <li><a href="/userpanel"><i class="fa fa-edit"></i>edit profile</a></li>
                <li><a href="{{route('logout')}}"><i class="fa fa-sign-out"></i>sing out</a></li>
            </ul>
            <ul class="menu-sidebar">
                <li><a href="{{route('aboutus')}}"><i class="fa fa-question-circle"></i>About Us</a></li>
                <li><a href="{{route('contact')}}"><i class="fa fa-send-o"></i>Contact</a></li>
                <li><a href="{{route('references')}}"><i class="fa fa-file-text"></i>References</a></li>
                <li><a href="{{route('faq')}}"><span class="material-icons">quiz</span>FAQ</a></li>
            </ul>
        </div><!-- // sidebar-stick -->
        <div class="clear"></div>
    </div><!-- // left-sidebar -->
</div>
